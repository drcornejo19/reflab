    ...
    <main className="min-h-screen overflow-hidden bg-[#020b14] text-white">
      <section className="relative flex min-h-screen flex-col items-center justify-center px-6 py-12">
        ...
        <div className="relative z-10 w-full max-w-6xl text-center">
          ...
          <nav className="mx-auto grid max-w-4xl grid-cols-2 gap-5 sm:grid-cols-3 lg:flex lg:items-center lg:justify-center lg:gap-10">
            {modules.map(({ href, label, Icon }, index) => (
              <Link
                key={href}
                href={href}
              ...
              >
                <div className="absolute inset-0 rounded-3xl bg-[#6fc11f] opacity-0 blur-2xl transition group-hover:opacity-15" />
                ...
                <span className="relative mt-4 text-[10px] font-black uppercase tracking-[0.35em] text-zinc-300 transition group-hover:text-[#6fc11f] sm:text-xs">
                  {label}
                </span>

                {index < modules.length - 1 && (
                  <div className="hidden h-14 w-px bg-zinc-700/80 lg:absolute lg:-right-5 lg:block" />
                )}
              </Link>
            ))}
          </nav>
