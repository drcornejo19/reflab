...
import { updateInstitutionGroup } from "@/lib/institutional/directory-server";
import {
...
export async function PATCH(
  ...
  try {
    ...
    if (!institutionId || !isInstitutionLifecycleStatus(body.status)) {
      return errorResponse("Los datos del grupo no son validos.", 400);
    }
    const result = await updateInstitutionGroup(
      institutionId,
      groupId,
      body.status
    );
